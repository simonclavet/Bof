#include "stdafx.h"
#include "DynamicUniformBufferExample.h"
...
void DynamicUniformBufferExample::run(
    HINSTANCE hInstance, 
    WNDPROC wndproc, 
    const std::vector<const char*>& args)
{
    ...
    m_vertices.attributeDescriptions =
    {
        ...
        vks::initializers::vertexInputAttributeDescription(
            VERTEX_BUFFER_BIND_ID, 1, VK_FORMAT_R32G32B32_SFLOAT, offsetof(Vertex, color)),
    };

    m_vertices.inputState = vks::initializers::pipelineVertexInputStateCreateInfo();
    m_vertices.inputState.vertexBindingDescriptionCount = (u32)(m_vertices.bindingDescriptions.size());
    m_vertices.inputState.pVertexBindingDescriptions = m_vertices.bindingDescriptions.data();
    m_vertices.inputState.vertexAttributeDescriptionCount = (u32)(m_vertices.attributeDescriptions.size());
    ...
    {
        size_t otherThing = ~(minUboAlignment - 1);
        m_dynamicAlignment = (m_dynamicAlignment + minUboAlignment - 1) & otherThing;
    }

    size_t bufferSize = OBJECT_INSTANCES * m_dynamicAlignment;

    m_uboDataDynamic.model = (glm::mat4*)alignedAlloc(bufferSize, m_dynamicAlignment);

    BOF_ASSERT(m_uboDataDynamic.model != nullptr);
    ...
    VK_CHECK_RESULT(m_vulkanDevice->createBuffer(
        ...
        &m_uniformBuffers.dynamic,
        bufferSize));

    m_uniformBuffers.dynamic.descriptor.range = sizeof(glm::mat4);

    VK_CHECK_RESULT(m_uniformBuffers.view.map());
    VK_CHECK_RESULT(m_uniformBuffers.dynamic.map());

    std::default_random_engine rndEngine((unsigned)time(nullptr));
    std::normal_distribution<float> rndDist(-1.0f, 1.0f);

    for (u32 i = 0; i < OBJECT_INSTANCES; i++)
    {
        m_rotations[i] = glm::vec3(rndDist(rndEngine), rndDist(rndEngine), rndDist(rndEngine)) * 2.0f * (float)M_PI;
        m_rotationSpeeds[i] = glm::vec3(rndDist(rndEngine), rndDist(rndEngine), rndDist(rndEngine));
    }

    //
    //const u32 glTFLoadingFlags = vkglTF::FileLoadingFlags::PreTransformVertices | vkglTF::FileLoadingFlags::PreMultiplyVertexColors | vkglTF::FileLoadingFlags::FlipY;
    ...
    //descriptorPoolSizes[1].descriptorCount = static_cast<u32>(m_cubes.size());
    
    descriptorPoolSizes[1].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC;
    ...
    descriptorPoolCI.poolSizeCount = static_cast<u32>(descriptorPoolSizes.size());
    descriptorPoolCI.pPoolSizes = descriptorPoolSizes.data();
    // Max. number of descriptor sets that can be allocated from this pool (one per object)
    descriptorPoolCI.maxSets = 2;// static_cast<u32>(m_cubes.size());
    ...
    writeDescriptorSets[0].pBufferInfo = &m_uniformBuffers.view.descriptor;
    ...
    writeDescriptorSets[1].pBufferInfo = &m_uniformBuffers.dynamic.descriptor;
    writeDescriptorSets[1].descriptorCount = 1;

    vkUpdateDescriptorSets(m_device, static_cast<u32>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, nullptr);
    ...
    // prepare pipelines
    {
        ...
        pipelineCI.pVertexInputState = &m_vertices.inputState;



        // Textured pipeline
        // Phong shading pipeline
        shaderStages[0] = loadShader(getShadersPath() + "base.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
        shaderStages[1] = loadShader(getShadersPath() + "base.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
    ...
    while (!quitMessageReceived)
    {
            ...
            !IsIconic(m_window))
        {
            ...
            result = m_swapChain.queuePresent(m_queue, m_currentBuffer, m_semaphores.renderComplete);
...
std::string DynamicUniformBufferExample::getWindowTitle()
{
...
void DynamicUniformBufferExample::createCommandBuffers()
{
...
void DynamicUniformBufferExample::destroyCommandBuffers()
{
    vkFreeCommandBuffers(m_device, m_cmdPool, static_cast<u32>(m_drawCmdBuffers.size()), m_drawCmdBuffers.data());
}

std::string DynamicUniformBufferExample::getShadersPath() const
{
    return "Data/shaders/";
}
...
VkPipelineShaderStageCreateInfo DynamicUniformBufferExample::loadShader(std::string fileName, VkShaderStageFlagBits stage)
{
...
void DynamicUniformBufferExample::updateOverlay()
{
    if (!m_settings.overlay)
        return;
...
void DynamicUniformBufferExample::drawUI(const VkCommandBuffer commandBuffer)
{
    if (m_settings.overlay) {
...
void DynamicUniformBufferExample::setupDPIAwareness()
{
...
void DynamicUniformBufferExample::handleMessages(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    switch (uMsg)
    {
    case WM_CLOSE:
...
void DynamicUniformBufferExample::setupDepthStencil()
{
...
void DynamicUniformBufferExample::setupFrameBuffer()
{
...
void DynamicUniformBufferExample::buildCommandBuffers()
{
    ...
    for (int32_t i = 0; i < m_drawCmdBuffers.size(); ++i)
    {
        ...
        for (u32 j = 0; j < OBJECT_INSTANCES; j++)
        {
            u32 dynamicOffset = j * (u32)m_dynamicAlignment;

            vkCmdBindDescriptorSets(
                m_drawCmdBuffers[i],
                VK_PIPELINE_BIND_POINT_GRAPHICS,
                m_pipelineLayout,
                0, 1, &m_descriptorSet, 1, &dynamicOffset);
...
void DynamicUniformBufferExample::windowResize()
{
    if (!m_prepared)
    {
        return;
...
void DynamicUniformBufferExample::handleMouseMove(int32_t x, int32_t y)
{
...
void DynamicUniformBufferExample::updateUniformBuffers()
{
    m_uboVS.projection = m_camera.matrices.perspective;
    m_uboVS.view = m_camera.matrices.view;

    memcpy(m_uniformBuffers.view.mapped, &m_uboVS, sizeof(m_uboVS));

    // Dynamic ubo with per-object model matrices indexed by offsets in the command buffer
    //u32 dim = static_cast<u32>(pow(OBJECT_INSTANCES, (1.0f / 3.0f)));
    u32 dimX = 50;
    u32 dimY = 3;
    u32 dimZ = 50;
    ...
    for (u32 x = 0; x < dimX; x++)
    {
        for (u32 y = 0; y < dimY; y++)
        {
            ...
            {
                u32 index = x * dimY * dimZ + y * dimZ + z;

                glm::mat4* modelMat = (glm::mat4*)(((u64)m_uboDataDynamic.model + index * m_dynamicAlignment));

                m_rotations[index] += m_frameTimer * m_rotationSpeeds[index];
    ...
    memcpy(m_uniformBuffers.dynamic.mapped, m_uboDataDynamic.model, m_uniformBuffers.dynamic.size);

    VkMappedMemoryRange memoryRange = vks::initializers::mappedMemoryRange();
    memoryRange.memory = m_uniformBuffers.dynamic.memory;
    memoryRange.size = m_uniformBuffers.dynamic.size;
...
DynamicUniformBufferExample* vulkanExampleBase = nullptr;
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int)
{
    ...
    vulkanExampleBase = new DynamicUniformBufferExample();
