...
class SimpleClock
{
    public:
        ...
        void Reset() { m_first = std::chrono::system_clock::now(); }
        
        Duration GetTimeDuration() const { return std::chrono::system_clock::now() - m_first; }

        template<typename T>
        auto GetDurationAs() const { return std::chrono::duration_cast<T>(GetTimeDuration()); }

        int64_t GetTimeNano() const { return GetDurationAs<std::chrono::nanoseconds>().count(); }
        int64_t GetTimeMicro() const { return GetDurationAs<std::chrono::microseconds>().count(); }
        double GetTimeMillis() const { return GetTimeMicro() / 1000.0; }
        double GetTimeSecs() const { return GetTimeMicro() / 1000000.0;}
